...
color tokens, and typography the figures use.

---

### Figure 6.1 — FLOP vs MAC equivalence

Build a static-with-tiny-chart D3 v7 figure that decomposes one element of `y = Wx + b` into its operations. Three regions, laid out left-to-right with CSS grid: a left panel labelled "FLOP view" holding two stacked card rows — FLOP 1 multiply (`a × b → t`) and FLOP 2 add (`result + t → result`); a center collapse arrow with the caption "collapses to"; a right panel labelled "MAC view" holding one ink-filled card showing the fused expression `result += a × b` with the note "one instruction · one cycle on M4 / M7 / A53". Beneath, an ochre-left-border callout with the rule of thumb (MACs ≈ FLOPs ÷ 2 for pure MAD work) and a small horizontal bar chart of the operation counts for a 256→128 fully connected layer: 32,768 MACs (red), 65,536 multiply-add FLOPs (ink), 128 bias-add FLOPs (ink). Use `d3.scaleLinear` for the bar chart, value labels at the bar ends, JetBrains Mono tick labels, and a single tooltip showing the operation count and the per-row note.

> Reference implementation: `d3/chapter-06-compute-fig-01.html`

---

### Figure 6.2 — Roofline model

Build a single-panel D3 v7 roofline plot for a Cortex-M7 at 400 MHz. Channels: x is arithmetic intensity (MACs per byte transferred) on `d3.scaleLog` with domain [0.25, 64]; y is achievable performance (MMAC/s) on `d3.scaleLog` with domain [10, 800]. Roofline as a `d3.line` path through `(0.25, 25)`, `(4, 400)`, and `(64, 400)` — the memory-bandwidth slope of 100 MMAC/(s·byte) meeting a 400 MMAC/s compute plateau at a ridge point at intensity 4. Annotate the slope segment with "memory-bandwidth ceiling" rotated about -32°, the plateau with "compute-bound plateau (400 MMAC/s sustained)", and the ridge with a 4-pixel ink dot. Plot three kernels as circles with labels next to each: A 64×64 matmul at (8, 350) in ink; B 3×3 conv 96×96×32 at (1.5, 100) in red; C 512×512 matmul at (0.5, 50) in red. Dashed vertical drop lines from each point to the x-axis. Light gridlines at log decades. Tooltip on hover: kernel name, intensity, achieved performance, one-sentence note.

> Reference implementation: `d3/chapter-06-compute-fig-02.html`

---

### Figure 6.3 — Processor-class ladder

Build a five-column ladder figure in D3 v7 showing pipeline depth, SIMD width, clock, peak int8 MMAC/s, and example chip for: Cortex-M0+, Cortex-M4, Cortex-M7, Cortex-A53, and an NPU tier. Use `d3.scaleBand` over the five tier names for the x-axis layout. At the top, render a row of "capability bars" — five filled rectangles whose heights grow left-to-right (16, 30, 44, 58, 72 px) and whose fills run from light gray through `--color-ink` to `--color-red` for the NPU. Below the bars, a tier name in EB Garamond. Below the names, five horizontal rows separated by 1px `--color-border` rules, one per attribute. Each cell has a primary line and a smaller secondary line in `--color-secondary` (e.g. peak shows "≈ 400 M" with "int8 SIMD via CMSIS-NN" underneath). Footer arrow spanning the full width with the caption "capability × cost × power — all rise together". Tooltip on hover over any capability bar: tier name, pipeline, SIMD, clock, peak, example chip.

> Reference implementation: `d3/chapter-06-compute-fig-03.html`

---

### Figure 6.4 — SIMD lane diagram

Build a two-row D3 v7 figure contrasting scalar and SIMD execution of four int8 MACs. Top row labelled "Scalar Cortex-M4 (no SIMD)": four equally-sized cycle boxes side by side, each containing two small 28×22 byte cells `aᵢ` and `bᵢ`, an `×` between them, `+=` after, and a wider 32-wide accumulator cell `acc`. Caption per cycle: "MUL + ADD". Right-side label: "4 cycles total". Bottom row labelled "SIMD Cortex-M4 (DSP extensions, SMLAD-style)": two stacked 32-bit registers `Ra` and `Rb`, each rendered as four equal-width lane boxes labelled `a₀…a₃` and `b₀…b₃`. Four short arrows descend from the lane pairs into one wide ink-filled accumulator bar holding the expression `acc += a₀·b₀ + a₁·b₁ + a₂·b₂ + a₃·b₃` (white text). Right-side label: "1 cycle". Below, a red 18-px EB Garamond badge: "4× throughput at identical clock". Marker arrowheads only — no animation beyond the existing CSS transition. Tooltip on the SIMD lane cells: "Lane i: Ra byte = aᵢ, Rb byte = bᵢ".

> Reference implementation: `d3/chapter-06-compute-fig-04.html`

---

### Figure 6.5 — Latency–accuracy Pareto plot

Build a single-panel D3 v7 scatter with thresholds. Channels: x is inference latency (ms) on `d3.scaleLog` over [100, 3000]; y is accuracy (%) on `d3.scaleLinear` over [80, 96]. Three points: A 80 M MAC at (1333, 92), B 35 M MAC at (583, 89), C 12 M MAC at (200, 84). A and B in `--color-ink`; C in `--color-red`. Connect them with a dashed Pareto frontier through the latency-sorted order. Draw a vertical dashed threshold at x = 250 ms labelled "250 ms deadline" and a horizontal dashed threshold at y = 88 labelled "88% accuracy floor". Shade the upper-left feasibility rectangle (x ≤ 250, y ≥ 88) at 8% red opacity with a centered italic caption "feasible region (≤ 250 ms AND ≥ 88%)". Light gridlines at axis ticks. Tooltip on each point: model id, MAC count + accuracy, latency, status ("fails deadline" / "fails accuracy"), one-sentence note about parameter count.

> Reference implementation: `d3/chapter-06-compute-fig-05.html`

---

### Figure 6.6 — Latency budget waterfall

Build an interactive D3 v7 waterfall of a 1-second duty cycle. Channels: x is time within the window (ms) on `d3.scaleLinear` [0, 1000]; y is current draw (mA) on `d3.scaleLinear` [0, 32]. Two filled rectangles in `--color-ink`: an inference block from x = 0 to inferDuration at 28 mA; a 1-px sleep strip from inferDuration to 1000 at the baseline. Red dashed line at y = 5 labelled "5 mA budget ceiling"; ink dashed line at y = computed average. Above the chart, a range slider (20–220 ms, step 2, default 80) drives the inference duration; the chart, average line, and verdict card update on input. Below the chart, a math block showing `Average = (28 × t + 0.002 × (1 − t)) / 1 s` with live numbers, then three cards: Budget (5.00 mA), time-weighted Average (live), and Verdict — "pass · X mA headroom" on ink or "fail · over by X mA" on red. Annotate the inference block "Inference burst: N ms × 28 mA"; annotate the sleep block "M ms sleep at ~2 µA — practically zero".

> Reference implementation: `d3/chapter-06-compute-fig-06.html`
