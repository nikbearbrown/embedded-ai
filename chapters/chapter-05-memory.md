...
![Four-tier pyramid of embedded memory: registers at the apex, SRAM below at 64 KB–2 MB and 1–2 cycle access, on-chip flash next at 512 KB–2 MB and 10–50 cycle access, external PSRAM or SDRAM at the base at MB–GB and 10–50× SRAM latency, each tier labeled with size, latency, volatility, and what lives there.](../images/chapter-05-memory-fig-01.png)
*Figure 5.1 — Embedded memory hierarchy. Each tier trades latency and cost for capacity.*
...
![Two-bank diagram. Flash bank holds firmware binary, model weights, biases, quantization scales and activation lookup tables, all read-only at runtime. SRAM bank holds heap, stack, globals, RTOS task control blocks, DMA buffers, activation tensors and scratch buffers. Arrows route each deployment artifact to the correct bank, with a callout noting that execute-in-place from flash works for weights but never for activations.](../images/chapter-05-memory-fig-02.png)
*Figure 5.2 — Flash vs SRAM routing. Flash holds what never changes; SRAM holds what changes during inference.*
...
![Line chart of intermediate tensor memory across six CNN layer positions. The cumulative naive allocation line climbs from 110 KB at the input to 2.27 MB by the final layer, while the buffer-reuse line tracks per-layer live tensor size and peaks at 1.15 MB on Conv1. The shaded area between them shows the savings.](../images/chapter-05-memory-fig-03.png)
*Figure 5.3 — Activation footprint timeline across CNN layers. Naive cumulative allocation vs buffer reuse.*
...
![Three side-by-side buffer diagrams. Left: in-place computation, a single buffer with input and output overlaid for ReLU, batch norm, residual-add. Middle: buffer reuse, two ping-pong buffers swapping across feedforward layers. Right: scratch pad, a main activation buffer plus a small tiled scratch region for im2col. Each panel labeled with when it applies and its memory cost.](../images/chapter-05-memory-fig-04.png)
*Figure 5.4 — Three activation-layout strategies: in-place, buffer reuse, scratch pad.*
...
![Stacked horizontal bar of the STM32H743's 1 MB SRAM budget after the arena was bumped to 350 KB. Segments in order: tensor arena 350 KB, communication buffers 200 KB, RTOS kernel and task stacks 120 KB, application heap 80 KB, globals 50 KB, remaining call-stack headroom 224 KB outlined in red. The 80 percent threshold line crosses inside the headroom segment, marking the overflow that crashes inference 80 ms in.](../images/chapter-05-memory-fig-05.png)
*Figure 5.5 — STM32H743 1 MB SRAM budget after arena bump. The headroom that fails under load.*
...
![Decision tree of the three memory failure modes. From the root "memory fails," three branches lead to "doesn't fit in flash" (with leaves quantize, prune, smaller architecture), "doesn't fit in SRAM" (with leaves reduce channels, swap to MobileNet, add PSRAM), and "insufficient margin above 80 percent" (with leaves reduce model, reduce app memory, upgrade hardware). Each leaf is annotated with expected savings.](../images/chapter-05-memory-fig-06.png)
*Figure 5.6 — Three memory failure modes. Which branch fired tells you which mitigation to reach for.*
...
### Figure 5.1 — Embedded memory hierarchy pyramid

Build a single-panel D3 v7 figure rendering a four-tier memory pyramid. Tiers top to bottom: registers (apex, no size), SRAM (64 KB to 2 MB, 1–2 cycle, volatile), on-chip flash (512 KB to 2 MB, 10–50 cycle, non-volatile, ~100K erase cycles), external PSRAM or SDRAM (MB to GB, 100s of cycles). Render each tier as a horizontal trapezoid stacked from narrow at the top to wide at the base. Fill each tier with a luminance step from `var(--color-ink)` at the apex through `#404040` and `#787878` to `#ADADAD` at the base. Inside each tier, show the tier name (Inter 700, white text) and the size and latency on a second line. On the right of the pyramid, render a column of contents annotations — "working set," "stack · heap · activations," "firmware · weights · LUTs," "overflow weights / buffers" — each aligned to the centroid of its tier. On the left, show a latency axis with the cycle counts. Tooltips on hover show the full tier description. Standalone HTML, D3 v7, inline CSS/JS, accessible, responsive.

> Reference implementation: `d3/chapter-05-memory-fig-01.html`

---

### Figure 5.2 — Flash vs SRAM routing

Build a D3 v7 figure showing a two-bank routing diagram. Left bank: a tall rectangle filled `var(--color-ink)` labeled FLASH with subtext "non-volatile · read-only at runtime." Right bank: a tall rectangle filled `#404040` labeled SRAM with subtext "volatile · writable · scarce." Between the two banks, render a centered column of eight deployment-artifact rectangles, each white with a `var(--color-secondary)` outline and JetBrains Mono label: firmware binary, model weights, biases + quant scales, activation LUTs, activation tensors, stack + heap, DMA buffers, RTOS TCBs + globals. The first four route by arrow to the flash bank; the last four route by arrow to the SRAM bank. Below the diagram, render a callout box with an ochre border reading "Execute-in-place from flash works for weights — never for activations." Tooltip on hover shows which bank each artifact routes to. Standalone HTML, D3 v7, inline CSS/JS, accessible, responsive.

> Reference implementation: `d3/chapter-05-memory-fig-02.html`

---

### Figure 5.3 — Activation footprint timeline: naive vs reuse

Build a two-series line chart of intermediate tensor memory across six CNN layer positions: Input (96×96×3) 110 KB, Conv1 (96×96×32), Pool1 (48×48×32), Conv2 (48×48×64), Pool2 (24×24×64), FC (10). Two y-series: naive cumulative (110, 1260, 1548, 2124, 2268, 2270 KB) drawn in `var(--color-red)` solid; buffer-reuse per-layer live (110, 1150, 288, 576, 144, 0 KB) drawn in `var(--color-ink)` dashed. y-domain [0, 2500] KB, zero baseline. Shade the area between the two lines using `var(--color-border)` at 0.5 opacity. Annotate the naive peak ("2.27 MB") at the FC point and the reuse peak ("1.15 MB") at the Conv1 point. Render layer shapes (e.g., "96×96×32") under each x-tick. Tooltip on each circle shows layer, shape, and KB. Standalone HTML, D3 v7, inline CSS/JS, accessible, responsive.

> Reference implementation: `d3/chapter-05-memory-fig-03.html`

---

### Figure 5.4 — Three activation-layout strategies

Build a three-panel D3 v7 figure rendering three buffer diagrams side by side. Panel 1 ("in-place"): one rectangle labeled "buffer B" with text "read in[i] → write back to in[i]" and a curved arrow showing the input being overwritten; subtitle "ReLU, BN, residual-add"; cost label "0 extra buffer." Panel 2 ("buffer reuse"): two rectangles side by side labeled A and B for layer 1, then a second row showing A reused for layer 2 output and B holding layer 2 input, with arrows showing the ping-pong; subtitle "feedforward layers, ping-pong"; cost label "2× largest tensor." Panel 3 ("scratch pad"): one wide rectangle labeled "main activation arena" plus a small ochre rectangle labeled "scratch"; subtitle "im2col tiles, GEMM workspace"; cost label "arena + tile budget." Each panel inside a bordered card with title and subtitle. Tooltip on hover for each buffer rectangle. Standalone HTML, D3 v7, inline CSS/JS, accessible, responsive.

> Reference implementation: `d3/chapter-05-memory-fig-04.html`

---

### Figure 5.5 — STM32H743 SRAM budget after arena bump

Build a single horizontal stacked bar chart sized to the STM32H743's 1024 KB SRAM. Segments in order: arena 350 KB (`var(--color-ink)`), comms 200 KB (`#404040`), RTOS 120 KB (`#787878`), heap 80 KB (`#A8A4A0`), globals 50 KB (`#D4D4D4`), stack headroom 224 KB (white fill with `var(--color-red)` 2px stroke). x-axis ticks at 0, 256, 512, 768, 1024 KB. Above the bar, draw a vertical dashed ochre line at 80% (819 KB) labeled "80% threshold." Inside each segment label show the segment name and KB (white text where the fill is dark, `var(--color-ink)` where light, red where headroom). Tooltip on hover shows the segment name, its KB, and its share of total. Standalone HTML, D3 v7, inline CSS/JS, accessible, responsive.

> Reference implementation: `d3/chapter-05-memory-fig-05.html`

---

### Figure 5.6 — Three memory failure modes decision tree

Build a D3 v7 tree-layout figure with a single root "memory fails" branching to three intermediate nodes: "doesn't fit in flash," "doesn't fit in SRAM," "insufficient margin." Each intermediate node branches to three leaves. Flash branch leaves: quantize f32→int8 (savings −75%), prune layers (−20–50%), smaller architecture (−40–80%). SRAM branch leaves: reduce channels/res (−50–75%), swap to MobileNet (−30–60%), add PSRAM (+10–50× latency). Margin branch leaves: reduce model (target <70%), reduce app memory, upgrade hardware. Use `d3.tree()` over a `d3.hierarchy()` for layout. Render the root and intermediate nodes with dark fills and white text; leaves with white fills and `var(--color-ink)` text; savings tags below each leaf in `var(--color-ochre)` bold. Tooltip on hover shows the trigger or detail for each node. Standalone HTML, D3 v7, inline CSS/JS, accessible, responsive.

> Reference implementation: `d3/chapter-05-memory-fig-06.html`
